        ...
        for child in parent.children:
            ...
            docs.append(Document(
                ...
                metadata={
                    ...
                    "raw_content":     str(child.raw_content or "")[:500],  # truncate for storage
                    "retrieved_content": child.retrieved_content or "",
...
class RAGPipeline:
    ...
    async def run_async(
        ...
        print(f"      → {total_children} child chunks after merge/split")

        print("[3/6] Enriching non-text chunks with LLM...")
        await self.enricher.enrich_all(parent_chunks)
        print("      → enrichment complete")

        print("[4/6] Building embedding content...")
        build_embedding_content(parent_chunks)
        print("      → embedding content built")

        print("[5/6] Creating LangChain documents...")
        docs = chunks_to_langchain_docs(parent_chunks)
        print(f"      → {len(docs)} documents created")

        print("[6/6] Embedding and storing...")
